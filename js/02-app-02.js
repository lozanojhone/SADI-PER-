...
'use strict';
const OFFICIAL_SADI_LOGO='assets/logo-sadi-peru.png';
...
async function resolveHeroSlidesForSave(){
 ...
 for(let i=0;i<3;i++){
  ...
  if(file){const ext=(file.name.split('.').pop()||'jpg').replace(/[^a-z0-9]/gi,'').toLowerCase();image=await uploadImageAsset(file,`sadi/hero/banner_${i+1}_${Date.now()}.${ext}`)}
